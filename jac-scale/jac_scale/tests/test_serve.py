...
class TestJacScaleServeDevMode:
    ...
    def _get_auth_token(self) -> str:
        ...
        assert response.status_code == 201
        return self._extract_data(response.json())["token"]

    def test_schedule_list_endpoint(self) -> None:
        """Test listing schedules returns static schedules from @restspec."""
        token = self._get_auth_token()
        response = requests.get(
            f"{self.base_url}/schedule/list",
            headers={"Authorization": f"Bearer {token}"},
            timeout=10,
        )
        assert response.status_code == 200, (
    ...
    def test_schedule_create_interval(self) -> None:
        """Test creating an interval schedule via API."""
        token = self._get_auth_token()
        response = requests.post(
            f"{self.base_url}/schedule/create",
            json={
    ...
    def test_schedule_create_cron(self) -> None:
    ...
    def test_schedule_create_date(self) -> None:
    ...
    def test_schedule_get_by_id(self) -> None:
    ...
    def test_schedule_get_nonexistent(self) -> None:
        """Test getting a schedule that doesn't exist."""
        token = self._get_auth_token()
        response = requests.get(
            f"{self.base_url}/schedule/nonexistent-id",
            headers={"Authorization": f"Bearer {token}"},
            timeout=10,
        )
        assert response.status_code == 404

    def test_schedule_requires_auth(self) -> None:
